...
const SHOW_OVERVIEW_AT_STARTUP = true;

// Bottom edge settings
const HOT_EDGE_PRESSURE_TIMEOUT = 1000; // ms
const PRESSURE_TRESHOLD = 200;
...
const Dock = GObject.registerClass(
  class Dock extends Dash.Dash {
    ...
    _toggle_dock() {
      ...
      if (Main.overview.visible || focusWindow?.fullscreen) {
        return;
      }

      this.is_visible() ? this._hide_dock() : this._show_dock();
    }
    ...
    _dock_refresh() {
      ...
      this.set_height(
        Math.min(
          this.get_preferred_height(this.work_area.width),
          this.max_dock_height
        )
      );
...
export default class DockFromDashExtension {
        ...
        switch (app_windows.length) {
          ...
          default:
            if (Main.overview.visible) {
              ...
              if (app_has_focus) {
                let next_index =
                  (app_focused_window_index + 1) % app_windows.length;
                this.app.activate_window(
                  app_windows[next_index],
  ...
  enable() {
    ...
    Main.layoutManager.connectObject(
      "hot-corners-changed",
      this._update_hot_edges.bind(this),
      this
    );
